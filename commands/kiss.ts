...
export default {
	data: new SlashCommandBuilder()
						...
						.addUserOption(option => option.setName('usuario').setDescription('quem você irá beijar').setRequired(false)),
  aliases: ['beijo'],
    ...
    if (!user) {
      return interaction.reply('lembre-se de selecionar um usuário válido para beijar!');
    }
    if (user == interaction.user) return interaction.reply('Você não pode se beijar!')
    const embed = new EmbedBuilder()
      .setTitle('Acabaram de se beijar, apaixonados!')
      .setColor('#9d00ff')
