...
export default {
	data: new SlashCommandBuilder()
						.setName('eval')
						.setDescription('Comando exclusivo para os desenvolvedores do bot').addStringOption(option => option.setName('code').setDescription('codigo para dar eval').setRequired(true)),
	aliases: ['e'],
	...
	slash_run:async (interaction) => {
		...
		try {
			const evaluated = await Promise.resolve(eval(code));
			await interaction.reply(inspect(evaluated, {
				depth: 0,
			})) 
		} catch (err:any) {
			const embed = new EmbedBuilder()
