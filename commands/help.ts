...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("comando")
        .setDescription("ajuda sobre um certo comando")
        .setRequired(false)
    ),
  aliases: ["ajuda"],
  run: async (client, message, args) => {
    ...
    if (!args.length) {
      ...
      return message.author
        ...
        .catch((error) => {
          console.error(
            `Não posso mandar mensagem na dm de ${message.author.tag}.\n`,
            error
          );
    ...
    if (command.aliases)
      data.push(`**pode ser usado como:** ${command.aliases.join(", ")}`);
    if (command.data?.description)
      data.push(`**Descrição:** ${command.data?.description}`);
  ...
  slash_run: async (interaction) => {
    ...
    if (!args?.length) {
      ...
      return interaction.user
        ...
        .catch((error) => {
          console.error(
            `Não posso mandar mensagem na dm de ${interaction.user.tag}.\n`,
            error
          );
